...
(function(exports) {
	...
	var xDomainCookie = function( iframe_path, namespace, xdomain_only, iframe_load_timeout_ms, secure_only, debug ){
		...
		function _set_local_cookie( cookie_name, cookie_value, expires_days ){
			...
			var sameSite = _withSameSiteNone() ? ";SameSite=None" : "";
			var cookie_val = cookie_name + "=" + cookie_value + "; expires="+d.toUTCString() + ";path=/;domain=impresa.pt" + sameSite + ";secure";
		...
		function _withSameSiteNone() {
			if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
				return true;
			}
			var isCorrectChrome;
			if (navigator.userAgent.match(/Chrome\/(\d*)/)) {
				isCorrectChrome = parseInt(navigator.userAgent.match(/Chrome\/(\d*)/)[1]) > 65;
			}
			var isCorrectSafari = (navigator.userAgent.toLowerCase().indexOf('safari/') > -1) &&
