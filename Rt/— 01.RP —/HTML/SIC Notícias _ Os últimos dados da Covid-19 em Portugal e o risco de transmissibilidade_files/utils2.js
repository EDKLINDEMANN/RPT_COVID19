...
function contentLoadedPubSapo(win, fn) {
...
function pubSAPO_getScrollingPosition() {
...
function pubSAPO_insertAfter(newElement,targetElement) {
...
function pubSAPO_ClassName(classname, node) {
...
function pubSAPO_elm(elm,nodeValue){
	
	var elPubSapo = document.getElementById(elm);
	
	if(!elPubSapo && !nodeValue)
	{
	
		var elPubSapo = getElementsByClassNameSapo(elm);
			elPubSapo = elPubSapo[0];
			
	}else if(!elPubSapo && nodeValue) {
	
	
		var elPubSapo = getElementsByClassNameSapo(elm);
		
			elPubSapo = eval(elPubSapo[nodeValue]);
...
function pubSAPO_nextElementSibling( el ) {
    do { el = el.nextSibling } while ( el && el.nodeType !== 1 );
    return el;
}

/* full proof code by @Gamboa */


		

function pubSAPO_addLoadEvent(func) { 
...
function pubSAPO_addResizeEvent(func) { 
...
function pubSAPO_addScrollEvent(func) { 
...
/* PUB SAPO */
var $pub = function( id ) { return document.getElementById( id ); };
